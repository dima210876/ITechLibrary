...
public class BorrowDao implements DefaultDao<Borrow>
{
    ...
    private static final String UPDATE_BORROW_STATUS_TO_RETURNED_SQL =
            "update borrows set borrow_status = 'returned' where id = ?";
    ...
    public boolean updateBorrowStatusToReturned(long borrowId)
    {
        PreparedStatement statement = null;
        Connection conn = null;
        try
        {
            conn = POOL.retrieveConnection();
            statement = conn.prepareStatement(UPDATE_BORROW_STATUS_TO_RETURNED_SQL);
        ...
        {
            LOGGER.error("SQLException while trying to update borrow status to 'returned': "
                    + e.getLocalizedMessage());
        }
        finally
        {
    ...
    public Optional<Borrow> create(Borrow borrow)
    {
        ...
        {
            conn = POOL.retrieveConnection();
            statement = conn.prepareStatement(CREATE_BORROW_SQL);
